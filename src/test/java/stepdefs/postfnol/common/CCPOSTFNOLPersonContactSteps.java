...
public class CCPOSTFNOLPersonContactSteps extends BaseTest {
	
	
	public void completePostFNOLPropertyOwnerForTestScenario()
	{
		
		String fieldValue = ExcelUtil.getTestDataValue("PostFnol_PropertyNewOwnerPrefix");
		if (fieldValue !=null)
			personContactDetailsPOM.selectPrefix(fieldValue);
		
		fieldValue = ExcelUtil.getTestDataValue("PostFnol_PropertyNewOwnerFirstName");
		if (fieldValue !=null)
			personContactDetailsPOM.setFirstName(fieldValue);
		
		fieldValue = ExcelUtil.getTestDataValue("PostFnol_PropertyNewOwnerLastName");
		if (fieldValue !=null)
			personContactDetailsPOM.setLastName(fieldValue);
		
		fieldValue = ExcelUtil.getTestDataValue("PostFnol_PropertyNewOwnerEmail");
		if (fieldValue !=null)
			personContactDetailsPOM.setEmail(fieldValue);
		
		fieldValue = ExcelUtil.getTestDataValue("PostFnol_PropertyNewOwnerLocationDesc");
		if (fieldValue !=null)
	...
	@Then("^I will be on person contact screen for Post FNOL$")
	...
	@Given("^I select \"([^\"]*)\" on person details screen for Post FNOL$")
	...
	@Given("^I select \"([^\"]*)\" from \"([^\"]*)\" on person details screen for Post FNOL$")
	...
	@Given("^I input \"([^\"]*)\" into the \"([^\"]*)\" box on person details screen for Post FNOL$")
	...
	@Then("^I complete the \"([^\"]*)\" details on the new person contact screen for Post FNOL$")
